...
import { Text, View, ImageBackground, TouchableOpacity, TextInput, ScrollView, Image, SafeAreaView, StyleSheet} from 'react-native';
import { FlatList } from "react-native-gesture-handler";

import { Categorias, destaque, Cantinas, bancod, produto, } from '../../constants';
...
export default function Principal({navigation}) {
    ...
    const renderItems = ({item}) => {
        return(
            <View key={item.id} className="font-bold ">
                <TouchableOpacity activeOpacity={1.0} className="my-4 mx-2.5" onPress={() => {navigation.navigate('Item', 
                {id: item.id, image: item.image, name: item.name, descricao: item.descricao, price: item.price,})}}>
                    <View className="rounded-xl bg-white shadow shadow-black/50 ">
                                    
                        <Image className="flex h-36 w-48 rounded-t-xl" source={item.image}></Image>
                        <Text className="p-2 text-xl font-bold">{item.name}</Text>

                    </View>

                    <Text className="flex text-lg font-semibold pl-2.5 pb-5 mt-1">{item.price}</Text>
                </TouchableOpacity>
            </View>
        )
    }
    ...
    const renderItem = ({ item }) => {
        return( 
            ...
            <TouchableOpacity  activeOpacity={1.0} className="flex-row flex ml-5 mt-3 mb-3 " onPress={() => navigation.navigate('Item',
            {id: item.id, image: item.image, name: item.name, descricao: item.descricao, price: item.price,})}>
    ...
    return (
        <View className="flex h-full bg-white">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className=" mt-10 mb-2">
                <View className="flex-row justify-start self-start gap-x-3 px-5">
                    {
                    Cantinas.map(e => (
                            <TouchableOpacity activeOpacity={0.8} onPress={() => setCantinaFilter(e.cantina)} className="flex justify-center shadow shadow-black/25 bg-white rounded-full mt-2"
                            style={[style.Boff, cantinas === e.cantina && style.Bon]}>
                                <Text className="px-2 py-0.5 font-medium" style={[style.Toff, cantinas === e.cantina && style.Ton]}>{e.cantina}</Text>
                            </TouchableOpacity>
                    ))
                    }
                </View>
            ...
            <View>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} >    
                    ...
                    Categorias.map(e => (
                        ...
                        <TouchableOpacity activeOpacity={0.8} onPress={() => setCategoriaFilter(e.status)} className="px-10 py-3 bg-white my-3 shadow shadow-black/25 rounded-lg " 
                        style={[style.Boff, categoria === e.status && style.Bon]} >

                            <Text className=" font-medium uppercase" style={[style.Toff, categoria === e.status && style.Ton]}>{e.status}</Text>
                        </TouchableOpacity>
